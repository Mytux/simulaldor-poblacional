...
with col_control1:
    ...
    R0_1 = st.slider("R0 - Tasa Neta de Reproducción", 0.1, 50.0, 31.2, key="r1")
    T_1 = st.slider(
        "T - Tiempo Generacional", 0.5, 20.0, 8.96, key="t1"
    )
    r1 = np.log(R0_1) / T_1
...
with col_control2:
    ...
    R0_2 = st.slider("R0 - Tasa Neta de Reproducción ", 0.1, 50.0, 31.2, key="r2")
    T_2 = st.slider(
        "T - Tiempo Generacional ", 0.5, 20.0, 4.5, key="t2"
    )
    r2 = np.log(R0_2) / T_2
...
N0 = st.sidebar.number_input(
    "Población inicial de hembras (N0)", value=10, min_value=1
)
